...
- **`"llm_judge": true`.** Free-form ADRs whose rules cannot be expressed as regex but are still code-relevant. In Claude Code, adr-kit can batch these into one Sonnet call from its pre-commit hook. In Codex/generic workflows, treat this as a signal to perform an explicit model review against the ADR decision text and the diff.
...
- `"llm_judge": true` — for nuanced rules that need a model's judgement.
  Deep review happens via `/adr-kit:judge` in a Claude Code session, or by explicitly reviewing the diff against the ADR decision text in Codex/generic workflows.
